...
def load_mgatk_output(output_dir, mito_length):
    # Load base coverage files for each nucleotide
...
def gather_possible_variants(base_coverage_dict, reference_file):
    ...
    observed_set = set([(pos, base) for pos, base in zip(non_zero_pos, non_zero_bases) if pos not in ref_N_positions])
...
variant_vmr = variant_var / (variant_mean + 1e-10)
